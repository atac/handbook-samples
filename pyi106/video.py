...
class FileLoader(QtCore.QThread):
    ...
    def run(self):
        ...
        with closing(IO()) as PktIO:
            out = {}
            RetStatus = PktIO.open(self.filename, FileMode.READ)
            if RetStatus != Status.OK:
                print "Error opening data file %s" % self.filename
                raise SystemExit
            ...
            while RetStatus == Status.OK:
                ...
                # Ignore non-video.
                if not bool(0x3F < PktIO.Header.DataType < 0x43):
                    continue
                ...
                for i in range(len(data) / 188):
                    ...
                    body.byteswap()
                    out[path].write(body.tostring())

                self.pos = PktIO.get_pos()[1]
